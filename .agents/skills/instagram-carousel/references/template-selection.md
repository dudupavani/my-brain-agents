...
1. Filtre pelas entradas `useWhen` do registro e pelos `requiredFields` do `contentModel`; um slot de imagem exige uma imagem real.
...
4. Aplique `overflowAction` quando a copy não couber. Se nenhum template servir, o processo volta para revisão editorial.
